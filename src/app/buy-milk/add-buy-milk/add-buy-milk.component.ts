...
@Component({
  selector: 'app-add-buy-milk',
  templateUrl: './add-buy-milk.component.html',
  styleUrls: ['./add-buy-milk.component.css']
})
export class AddBuyMilkComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.itemService.addItemLine(this.buyMilkForm.value).subscribe(() => { 
      ...
      this.ngZone.run(() => {
        this.router.navigateByUrl('/buy-milk');
      });
    });
  }
}
